...
void PathPlan::ini_path_data(const pathInitData &pathInit) {
    cmd_pos = pathInit.cmd_pos;
    remaining_pos = pathInit.cmd_pos;

    v0 = fabs(pathInit.v0);
    Vmax = fabs(pathInit.VMax);
    ve = fabs(pathInit.ve);
    maxAcc = fabs(pathInit.max_acc);
    Jerk = fabs(pathInit.Jerk);
...
bool PathPlan::pathBusy() // 判断是否在规划中
{
    if (path_busy)
        return true;
    else
...
void PathPlan::judge_path_condition() // 规划前进行初始条件判断
{
    ...
    if (fabs(ve - vs) > coverAccuracy) { // 初速度和末速度不相等的情况下
        ...
        float cal_minDis = (vs + 0.5 * Jerk * pow(deltaT, 2)) * 4.0 * deltaT;
        float cal_vmin = vs + Jerk * pow(deltaT, 2);
        if (cal_minDis > fabs(cmd_pos) || cal_vmin > Vmax || Jerk * deltaT > maxAcc) { // 距离太短或最大加速度太小或最大速度太小，退出S型速度规划
            can_do_path = false;
        } else
            can_do_path = true; // 可以S型速度规划
    }
}

bool PathPlan::performPath(bool path_pause) // 执行规划
{
    if (!path_busy) {
        return false;
    }
    if (can_do_path) { // S型规划模式
        ...
        if (dec_finished) {
            ...
            if (path_pause_cmd & !decInit) {
                if (val_Auv > 0) { // 减速初始化
                    float cal_vmax, cal_maxAcc, t1_tmp, t2_tmp, moveDis;
...
void PathPlan::doMotionPlan(bool doPath) // 进入运动模式
{
...
bool PathPlan::preView(enumType motionType) // 前瞻下一时刻
{
    static float current_vs = 0, current_acc = 0, cal_vmax = 0, move_dis = 0;
    switch (motionType) {
    ...
    if (cal_vmax > Vmax || current_acc > maxAcc)
        return false;
    else if (((moveDir * cal_vmax * deltaT) / (cmd_pos + dec_move_pos)) > maxProportion)
        return false;
    else {
        move_dis += moveDir * (current_vs * current_acc / Jerk + 0.5 * current_acc * pow(current_acc / Jerk, 2) - Jerk * pow(current_acc / Jerk, 3) * 1.0 / 6.0);
        float cur_remaining_pos = remaining_pos - move_dis;
        float cal_maxAcc = sqrt(fabs(cal_vmax - ve) * Jerk);
        if (cal_maxAcc > maxAcc)
            cal_maxAcc = maxAcc;
        cal_maxAcc = (static_cast<int>((cal_maxAcc + coverAccuracy) / (Jerk * deltaT))) * Jerk * deltaT;
        float t1_tmp = cal_maxAcc / Jerk;
        float t2_tmp = 0;
        if (fabs(cal_maxAcc) > coverAccuracy)
            t2_tmp = (fabs(cal_vmax - ve) - pow(cal_maxAcc, 2) / Jerk) / cal_maxAcc;
        float moveDis = (ve + cal_vmax) * t1_tmp + (cal_vmax + ve) * t2_tmp / 2;
        if (fabs(moveDis) <= fabs(cur_remaining_pos))
            return true;
        else
            return false;
    }
...
void PathPlan::preFicure() // 全部前瞻完,结束前瞻
{
    float cal_acc_dis = moveDir * (vs * val_Auv / Jerk + 0.5 * val_Auv * pow(val_Auv / Jerk, 2) - Jerk * pow(val_Auv / Jerk, 3) * 1.0 / 6.0);
    float moveDis, cal_vmax, cal_maxAcc, t1_tmp, t2_tmp;
    get_time_point(ve, cal_vmax, cal_maxAcc, t1_tmp, t2_tmp, moveDis);
    ficureTime[0] = fabs(moveDis - fabs(remaining_pos - cal_acc_dis)) / cal_vmax;
    get_ficure_point(cal_vmax, ve, t1_tmp, t2_tmp);
}

void PathPlan::get_time_point(const float vEnd, float &cal_vmax, float &cal_maxAcc, float &t1_tmp, float &t2_tmp, float &moveDis) // 获取到达最大速度的时间,最大加速度,以及后半程各阶段时间
{
...
void PathPlan::get_ficure_point(const float vBegin, const float vEnd, const float t1_tmp, const float t2_tmp) // 获取后半程时间,位移,速度,加速度节点信息
{
    if (vBegin > vEnd)
        ficureMoveDir = 1;
    else
...
void PathPlan::moveFicurePos(enumType motionType, const float timeTmp) {
    switch (motionType) {
...
void PathPlan::movePreViewPos(enumType motionType) // 下一时刻运动计算
{
    switch (motionType) {
...
void PathPlan::get_move_msg(float &curProportion, float &curPos_m, float &curVs_m, float &curAcc_m, float &curTime_m) // 获得当前所走位移占比、总位移、速度、加速度、时间
{
